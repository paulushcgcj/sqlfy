...
class TestInsightsReport:
    ...
    def test_total_finding_count(self, sample_report):
        total = (
            len(sample_report.errors())
            + len(sample_report.warnings())
            + len(sample_report.infos())
        )
...
class TestSampleFindings:
    ...
    def test_nullable_fk_audit_changed_by(self, sample_report):
        ...
        findings = [
            f
            for f in sample_report.by_code("NULLABLE_FK")
...
class TestMissingFKCandidate:
    ...
    def test_pk_column_named_id_not_flagged(self):
        ...
        report = InsightsEngine.analyse(state)
        fk_candidates = [
            f for f in report.by_code("MISSING_FK_CANDIDATE") if f.column == "THING_ID"
        ]
...
def _build_multicommunity_state() -> SchemaState:
    ...
    def _fk(
        from_tbl: str, to_tbl: str, col: str = "ID", name: str | None = None
    ) -> None:
        rels.append(
            RelationshipState(
    ...
    comm_2 = [
        _table("CUST.CUSTOMER", "CUST", "CUSTOMER"),
    ...
    comm_3 = [
        _table("HR.EMPLOYEE", "HR", "EMPLOYEE"),
    ...
    _fk(
        comm_3[1], comm_3[0]
    )  # DEPARTMENT → EMPLOYEE (actually EMPLOYEE belongs to dept but FK wise let's say EMPLOYEE references DEPARTMENT)
    _fk(comm_3[0], comm_3[1])  # EMPLOYEE → DEPARTMENT
    _fk(comm_3[2], comm_3[1])  # ROLE → DEPARTMENT

    # Cross-community surprising joins
    _fk(
        comm_1[0], comm_2[0], col="CUSTOMER_ID", name="fk_order_customer"
    )  # ORDER → CUSTOMER (expected)
    _fk(
        comm_1[3], comm_3[0], col="APPROVED_BY", name="fk_shipment_approver"
    )  # SHIPMENT → EMPLOYEE (surprising)
...
class TestInsightsReportNewFields:
    ...
    def test_without_communities_new_fields_still_populated(self):
        ...
        assert (
            len(report.god_tables) > 0
        )  # god tables detected regardless of communities
