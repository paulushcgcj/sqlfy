...
_MIGRATION_V2 = (
    "CREATE TABLE orders (id NUMBER PRIMARY KEY, user_id NUMBER REFERENCES users(id));"
)
...
def _write_migrations(tmp: Path, migrations: dict[str, str]) -> list[dict]:
    ...
    return [
        {"filename": fname, "sql": sql} for fname, sql in sorted(migrations.items())
    ]
...
def test_diff_versions_v1_to_v2_adds_table(tmp_path, capsys):
    ...
    assert (
        data.get("stats", {}).get("tablesAdded", 0) > 0
        or len(data.get("tableChanges", [])) > 0
    )
...
def test_diff_versions_text_output(tmp_path, capsys):
    ...
    args = _make_args(
        migrations_dir=str(tmp_path), from_version="1", to_version="2", format="text"
    )
    cmd_diff_versions(**vars(args))

    captured = capsys.readouterr()
    assert (
        "SCHEMA STATE DIFF" in captured.out
        or "tablesAdded" in captured.out.lower()
