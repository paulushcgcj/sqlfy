...
class SchemaState:
    ...
    sqlfy_version
        The version of the sqlfy tool that generated this snapshot.
    tables
    ...
    def to_manifest(self) -> str:
        ...
        model = _SchemaManifest(
            ...
            node_count=self.stats.get("table_count", 0)
            + self.stats.get("sequence_count", 0),
    ...
    def get_table(self, name: str) -> TableState | None:
        ...
        name_up = name.upper()
        return self.tables.get(name_up) or next(
            (t for t in self.tables.values() if t.name == name_up), None
        )
    ...
    def tables_in_schema(self, schema: str) -> list[TableState]:
        return [
            ...
            if (t.schema or "").upper() == schema.upper()
        ]

    def orphan_tables(self) -> list[TableState]:
        """Tables with no FK relationships in or out."""
        rel_tables = {r.from_table for r in self.relationships} | {
            r.to_table for r in self.relationships
        }
    ...
    def tables_referencing(self, full_name: str) -> list[TableState]:
        ...
        names = {
            r.from_table for r in self.relationships if r.to_table == full_name.upper()
        }
    ...
    def tables_referenced_by(self, full_name: str) -> list[TableState]:
        ...
        names = {
            r.to_table for r in self.relationships if r.from_table == full_name.upper()
        }
        return [self.tables[n] for n in names if n in self.tables]
...
class SchemaStateBuilder:
    ...
    def from_graph(
        ...
        mig_history = [
            MigrationStep(version=m.version, description=m.description)
        ...
        fingerprint_src = json.dumps(
            {
                k: {
                    ...
                    "cons": [c.type for c in t.constraints],
                }
    ...
    def _build_relationships(graph: SchemaGraph) -> list[RelationshipState]:
        ...
        for e in graph.edges:
            ...
            pk = next(
                (
                    c
                    for c in (from_table.constraints if from_table else [])
