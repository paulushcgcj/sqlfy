...
def extract_column_lineage(
    ...
    for _table_id, table in schema_graph.tables.items():
...
def _normalize_table_name(table_name: str, schema_graph: SchemaGraph) -> str:
    ...
    for full_name in schema_graph.tables:
...
def find_god_columns(
    ...
    for _col_id, col_lineage in lineage.items():
...
def format_lineage_text(
    ...
    if not deps:
        ...
        for _i, dep in enumerate(deps[:max_display]):
            lines.append(f"  → {dep.full_name}")

            # Find the edge for this dependency
            for edge in col_lineage.edges:
                if direction == 'downstream' and edge.target == dep or direction == 'upstream' and edge.source == dep:
