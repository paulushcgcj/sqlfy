...
def cmd_lint(
    *,
    ...
    if not SQLFLUFF_AVAILABLE:
        ...
        print("Install with: pip install 'sqlfy-cli[lint]'", file=sys.stderr)
        sys.exit(1)
    ...
    if p.is_file():
        ...
        if fix:
            try:
                fixed = fix_migration(
                    original, p.name, dialect=dialect, config_path=config
                )
        ...
        if result.score < (min_score or 0):
            print(
                f"\nError: Score {result.score} is below minimum {min_score}",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
        if fix:
            ...
            for sql_file in sql_files:
                try:
                    ...
                    fixed = fix_migration(
                        original, sql_file.name, dialect=dialect, config_path=config
                    )
                    if fixed != original:
            ...
            write_output(
                format_directory_json(results)
                if fmt == "json"
                else format_directory_text(results),
                out,
            )
            failed = [r for r in results if r.score < (min_score or 0)]
            if failed:
                print(
                    f"\nError: {len(failed)}/{len(results)} files below minimum score {min_score}",
                    file=sys.stderr,
                )
                sys.exit(1)
            else:
                print(
                    f"\nApplied fixes to {changed}/{len(sql_files)} files",
                    file=sys.stderr,
                )
        else:
            results = lint_directory(
...
def cmd_validate(
    *,
    ...
    if not migrations_path.is_dir():
        print(
            f"Error: migrations directory not found: {migrations_path}", file=sys.stderr
        )
        return 1
    ...
    fmt = (format or "text").lower()
    write_output(
        ordering.format_json(report)
        if fmt == "json"
        else ordering.format_text(report, show_suggestions=True),
        out,
    )
...
def cmd_naming(
    *,
    ...
    report = naming.validate_naming(
        migrations_path, pattern=pattern, max_length=max_len
    )

    fmt = (format or "text").lower()
    write_output(
        naming.format_json(report) if fmt == "json" else naming.format_text(report), out
    )
    ...
    *,
    ...
    try:
        ...
        if critical_path and analysis.critical_path:
            ...
            print(
                f"  ({len(analysis.critical_path)} migrations must run sequentially)",
                file=sys.stderr,
            )

        error_count = sum(1 for issue in analysis.issues if issue.severity == "error")
        warning_count = sum(
            1 for issue in analysis.issues if issue.severity == "warning"
        )
        if error_count > 0:
            return 1
    ...
    *,
    ...
    if unused_columns:
        ...
        if fmt == "json":
            ...
            for col, refs in god_cols:
                ...
                if col.id in lineage:
                    ...
                    lines.append(
                        f"    Downstream columns: {len(col_lineage.downstream)}"
                    )
                lines.append("")
            if not god_cols:
                lines.append("  (none)")
        ...
        if col_id not in lineage:
            ...
            print(
                "Hint: Use format TABLE.COLUMN (e.g., APP.USERS.EMAIL)", file=sys.stderr
            )
            sys.exit(1)
        ...
        if fmt == "json":
            ...
            output = format_lineage_mermaid(
                col_id, lineage, direction=direction, max_depth=max_depth
            )
        else:
        ...
        deps = (
            col_lineage.upstream if direction == "upstream" else col_lineage.downstream
        )
...
def cmd_classify(
    *,
    ...
    fmt = (format or "text").lower()
    write_output(
        format_json(classifications)
        if fmt == "json"
        else format_text(classifications, group_by=group_by),
        out,
    )
    return 0
...
def cmd_safety(
    *,
    ...
    fmt = (format or "text").lower()
    write_output(
        format_json(scores) if fmt == "json" else format_text(scores, verbose=verbose),
        out,
    )
...
def cmd_cost(
    *,
    ...
    if no_recursive:
        ...
        print(
            f"Loaded {len(files)} migration file(s) from {p} (no-recursive)",
            file=sys.stderr,
        )
    else:
    ...
    write_output(
        format_json(results)
        if fmt == "json"
        else format_text(results, verbose=verbose, weight_profile=weight_profile),
        out,
    )
    return 0
    ...
    *,
    ...
    if extra_patterns:
        try:
            with open(extra_patterns, encoding="utf-8") as f:
                ...
                extra_patterns_dict = {
                    str(k): [str(p) for p in v] for k, v in extra_data.items()
                }
        except Exception as e:
    ...
    if fmt == "json":
        ...
        if not filtered_findings:
            ...
            print(
                f"Found {len(filtered_findings)} PII columns across {result.pii_table_count} tables.",
                file=sys.stderr,
            )
    else:
        # Text output
        ...
        a(
            f"PII Scan — {result.tables_scanned} tables, {result.columns_scanned} columns scanned"
        )
        ...
        if high_confidence:
            ...
            for finding in sorted(
                high_confidence, key=lambda x: (x.table_name, x.column_name)
            ):
                categories_str = ", ".join(finding.pii_categories)
                a(
                    f"  {finding.table_name}.{finding.column_name:<25} {categories_str:<20} confidence={finding.confidence:.2f}"
                )
            a("")
        ...
        if medium_confidence:
            ...
            for finding in sorted(
                medium_confidence, key=lambda x: (x.table_name, x.column_name)
            ):
                categories_str = ", ".join(finding.pii_categories)
                a(
                    f"  {finding.table_name}.{finding.column_name:<25} {categories_str:<20} confidence={finding.confidence:.2f}"
        ...
        for finding in filtered_findings:
            table_pii_counts[finding.table_name] = (
                table_pii_counts.get(finding.table_name, 0) + 1
            )

        sorted_tables = sorted(
            table_pii_counts.items(), key=lambda x: x[1], reverse=True
        )
        if sorted_tables:
