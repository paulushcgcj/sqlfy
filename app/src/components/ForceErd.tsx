/**
 * sqlfy — src/components/ForceErd.tsx
 *
 ...
 *  • Click — clicking a node selects it (calls onSelectTable prop)
 ...
 *  • Dark-mode aware — reads CSS variables from the document root
...
export function ForceErd({ graph, selectedTable, onSelectTable, height = 360 }: Props) {
  ...
  edges.forEach(e => { connectedIds.add(e.fromTable); connectedIds.add(e.toTable); });
  ...
  useEffect(() => {
    ...
    // Prevent node drag from triggering zoom
    ...
    function getComponents(): string[][] {
      ...
      for (const n of nodeData) {
        ...
        while (stack.length) {
          ...
          adjList.get(cur)?.forEach(nb => { if (!visited.has(nb)) stack.push(nb); });
        }
        comps.push(comp);
      }
      return comps;
    ...
    // Only draw blobs for non-trivial components (≥ 2 nodes)
    const islandPaths  = islandGroup.selectAll<SVGPathElement, string[]>('path')
    ...
    const linkSel = edgeGroup.selectAll<SVGPathElement, LinkDatum>('path')
      .data(linkData)
      .join('path')
        .attr('fill', 'none')
    ...
    linkSel
      .on('mouseover', (_event, d) => {
        const label = d.constraintName
          ...
          : d.onDelete ? `ON DELETE ${d.onDelete}` : 'FK';
    ...
    const nodeSel   = nodeGroup.selectAll<SVGGElement, NodeDatum>('g')
      .data(nodeData)
      .join('g')
        .attr('class', 'node')
        .attr('cursor', 'pointer')
    ...
    nodeSel.append('rect')
      ...
      .attr('stroke-width', d => d.isOrphan ? 1 : 1)
    ...
    // Orphan badge
    nodeSel.filter(d => d.isOrphan)
      .append('text')
        .attr('x', NODE_W - 8).attr('y', 14)
    ...
    function highlightNeighbours(hoveredId: string | null) {
      if (!hoveredId) {
        ...
        linkSel.attr('stroke', pal.edgeColor).attr('opacity', 0.7)
               .attr('marker-end', 'url(#arrow)');
        return;
      }
    ...
    // Click on background → deselect highlight
    ...
    const drag = d3.drag<SVGGElement, NodeDatum>()
      ...
      .on('drag', (ev: d3.D3DragEvent<SVGGElement, NodeDatum, NodeDatum>, d) => { d.fx = ev.x; d.fy = ev.y; })
      .on('end', (ev: d3.D3DragEvent<SVGGElement, NodeDatum, NodeDatum>, _d) => {
        ...
        // Uncomment to unpin after drag: _d.fx = null; _d.fy = null;
      });
    ...
    // ── Tick ──────────────────────────────────────────────────────────
    sim.on('tick', () => {
      // Edges: bezier curve offset so bidirectional edges don't overlap
      linkSel.attr('d', (l) => {
        ...
        const ty = (t.y ?? 0);
        // Shorten the path so the arrowhead sits on the node border
      ...
      nodeSel.attr('transform', d =>
        `translate(${(d.x ?? 0) - NODE_W / 2}, ${(d.y ?? 0) - NODE_H / 2})`
      );
  ...
  useEffect(() => {
    ...
    svg.selectAll<SVGGElement, NodeDatum>('.node').each(function(d) {
      ...
      d3.select(this).select('rect')
        .attr('fill',   isSel ? pal.nodeSelFill  : (d.isOrphan ? pal.orphanFill  : pal.nodeFill))
        .attr('stroke', isSel ? pal.nodeSelStroke : (d.isOrphan ? pal.orphanStroke : pal.nodeStroke))
        .attr('stroke-width', isSel ? 2 : 1);
      d3.select(this).select('.node-label')
        .attr('fill', isSel ? pal.labelSel : pal.labelColor);
    });
  }, [selectedTable, isDark]);

  // ── Fit-to-view helper (exposed via button) ───────────────────────────
  ...
  return (
    <div style={{ position: 'relative', width: '100%', background: palette(isDark).bgColor }}>
      ...
      <div style={{
        ...
        {[
          ...
          <button key={label} onClick={fn} title={title} style={{
            ...
            borderRadius: 5, color: isDark ? '#9ca3af' : '#6b7280',
          }}>
            {label}
          </button>
        ))}
      ...
      <div style={{
        ...
        <span>── FK edge</span>
        <span style={{ borderBottom: '1.5px dashed currentColor', paddingBottom: 1 }}>
          ╌ orphan
        </span>
