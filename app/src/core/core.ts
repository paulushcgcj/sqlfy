// ─────────────────────────────────────────────
// sqlfy — src/core/core.ts
// Pure logic module — fully typed TypeScript port of cli/core.js
...
export function splitStmts(sql: string): string[] {
  const r: string[] = [];
  let depth = 0,
    cur = '',
    inStr = false;
...
export function extractParen(str: string): string {
...
export function parseName(str: string): { schema: string | null; name: string; full: string } {
...
export function parseDataType(s: string): {
  type: string;
...
export function typeStr(col: Pick<Column, 'type' | 'precision' | 'scale'>): string {
...
export function parseColDef(def: string): Column | null {
  ...
  const dm = rest.match(
    /DEFAULT\s+(.+?)(?=\s+(?:NOT\s+NULL|NULL|CONSTRAINT|PRIMARY|UNIQUE|REFERENCES|ENABLE|DISABLE)|$)/i,
  );
  if (dm) {
    defVal = dm[1].trim();
  ...
  if (rm) {
    references = { table: parseName(rm[1]).full, column: rm[2].toUpperCase() };
  }
...
export function parseConstraint(def: string): Constraint | null {
  def = def.replace(/\s+/g, ' ').trim();
  let cname: string | null = null;
  const cn = def.match(/^CONSTRAINT\s+"?(\w+)"?\s+/i);
  if (cn) {
    cname = cn[1].toUpperCase();
    def = def.slice(cn[0].length).trim();
  }

  if (/^PRIMARY\s+KEY/i.test(def)) {
    const columns = extractParen(def)
      .split(',')
      .map((c) => c.trim().replace(/"/g, '').toUpperCase())
      .filter(Boolean);
    return { name: cname, type: 'primary_key', columns };
  }
  ...
  if (/^UNIQUE/i.test(def)) {
    const columns = extractParen(def)
      ...
      .map((c) => c.trim().replace(/"/g, '').toUpperCase())
      .filter(Boolean);
    return { name: cname, type: 'unique', columns };
  }

  if (/^FOREIGN\s+KEY/i.test(def)) {
    const fromCols = extractParen(def)
      .split(',')
      .map((c) => c.trim().replace(/"/g, '').toUpperCase())
      .filter(Boolean);
...
export function handleCreateTable(stmt: string, version: string, tables: Map<string, Table>): void {
...
export function handleAlterTable(stmt: string, version: string, tables: Map<string, Table>): void {
...
export function handleCreateIndex(stmt: string, version: string, tables: Map<string, Table>): void {
  const m = stmt.match(
...
export function handleCreateSeq(stmt: string, version: string, seqs: Map<string, Sequence>): void {
...
export function handleComment(stmt: string, tables: Map<string, Table>): void {
...
export function parseFlywayVer(filename: string): {
  version: string;
  description: string;
  order: number;
} {
...
export function applyMigrations(files: MigrationFile[]): SchemaGraph {
  const sorted = [...files]
  ...
  const migHist: { version: string; description: string }[] = [];

  for (const file of sorted) {
    migHist.push({ version: file.version, description: file.description });
    const stmts = splitStmts(stripComments(file.sql));
    ...
    for (const stmt of stmts) {
      ...
      else if (up.startsWith('CREATE INDEX') || up.startsWith('CREATE UNIQUE INDEX'))
        handleCreateIndex(stmt, file.version, tables);
...
export function buildChunks(graph: SchemaGraph): VectorChunk[] {
  ...
  for (const [, t] of tables) {
    ...
    L.push(
      `Schema: ${t.schema ?? 'default'} | Created: V${t.createdIn}${t.modifiedIn.length ? ` | Modified: V${t.modifiedIn.join(', V')}` : ''}`,
    );
    L.push('', 'COLUMNS:');
    ...
    for (const col of t.columns) {
      ...
      L.push(
        `  ${col.name}: ${typeStr(col)}${flags.length ? ` [${flags.join(', ')}]` : ''}${comment ? ` -- ${comment}` : ''}`,
      );
    }
    ...
    if (outE.length) {
      ...
      for (const e of outE)
        L.push(
          `  ${e.fromCols.join(',')}) → ${e.toTable}(${e.toCols.join(',')})${e.onDelete ? ` ON DELETE ${e.onDelete}` : ''}${e.constraintName ? ` [${e.constraintName}]` : ''}`,
        );
    }
    if (inE.length) {
      L.push('', 'REFERENCED BY:');
      for (const e of inE)
        L.push(`  ${e.fromTable}.${e.fromCols.join(',')} → ${e.toCols.join(',')}`);
    }
    if (t.indexes.length) {
      L.push('', 'INDEXES:');
      for (const idx of t.indexes)
        L.push(
          `  ${idx.name}: (${idx.columns.join(', ')})${idx.unique ? ' UNIQUE' : ''} [V${idx.createdIn}]`,
        );
    }
    if (cks.length) {
  ...
  const schemas = [
    ...new Set([...tables.values()].map((t) => t.schema).filter((s): s is string => s !== null)),
  ];
  ...
  const sumL = [
    ...
    'RELATIONSHIP MAP:',
    ...edges.map(
      (e) => `  ${e.fromTable}.${e.fromCols.join(',')} → ${e.toTable}.${e.toCols.join(',')}`,
    ),
    '',
    'TABLE ROLES:',
    ...[...tables.values()].map((t) => {
  ...
  for (const tname of tableNames) {
    ...
    for (const e of out)
      relL.push(
        `  ──FK──▶ ${e.toTable} via ${e.fromCols.join(',')}${e.onDelete ? ` [ON DELETE ${e.onDelete}]` : ''}`,
      );
...
export function computeLayout(
  ...
  edges: Edge[],
  { width = 580, height = 220 }: { width?: number; height?: number } = {},
  ...
  for (const [level, tbls] of byLevel) {
    ...
    tbls.forEach((t, i) => {
      pos.set(t, { x: ((i + 1) / (tbls.length + 1)) * width, y });
    });
  }
