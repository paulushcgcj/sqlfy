...
import type {
  MigrationFile, Column, Constraint, Table, Sequence,
  Edge, SchemaGraph, VectorChunk, LayoutPoint,
} from './types';
...
export function parseDataType(s: string): { type: string; precision: number | null; scale: number | null } {
...
export function parseColDef(def: string): Column | null {
  ...
  let nullable = true, defVal: string | null = null;
  let primaryKey = false, unique = false;
...
export function parseConstraint(def: string): Constraint | null {
  ...
  if (/^PRIMARY\s+KEY/i.test(def)) {
    const columns = extractParen(def).split(',')
...
export function parseFlywayVer(filename: string): { version: string; description: string; order: number } {
...
export function buildChunks(graph: SchemaGraph): VectorChunk[] {
  ...
  for (const [, t] of tables) {
    ...
    chunks.push({
      ...
      content: L.join('\n'),
      meta: {
        tableName: t.name, schema: t.schema, columnCount: t.columns.length,
        hasPK: !!pk, fkCount: fks.length, referencedBy: inE.length,
        indexCount: t.indexes.length, createdIn: t.createdIn, pkCols: pk?.columns ?? [],
      },
  ...
  const schemas    = [...new Set([...tables.values()].map(t => t.schema).filter((s): s is string => s !== null))];
